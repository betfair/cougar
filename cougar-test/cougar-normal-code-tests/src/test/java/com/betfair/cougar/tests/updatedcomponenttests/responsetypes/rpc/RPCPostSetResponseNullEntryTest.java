...
// Originally from UpdatedComponentTests/ResponseTypes/RPC/RPC_Post_SetResponse_NullEntry.xls;
...
/**
 * Ensure that when a Batched JSON request is performed against Cougar, passing a Set with a null entry in the post body, it is correctly de-serialized, processed, returned the correct response.
 */
public class RPCPostSetResponseNullEntryTest {
    @Test
    public void doTest() throws Exception {
        ...
        AssertionUtils.multiAssertEquals("{\"id\":1,\"result\":[null,\"Value a\"],\"jsonrpc\":\"2.0\"}", map5.get("response1"));
        AssertionUtils.multiAssertEquals("{\"id\":2,\"result\":[null,\"Value c\"],\"jsonrpc\":\"2.0\"}", map5.get("response2"));
