...
// Originally from UpdatedComponentTests/StandardValidation/SOAP/SOAP_TestLargeGet_ERRORHANDLING_INVALIDTYPE.xls;
...
/**
 * Ensure that when a SOAP request is received with a String parameter when int was expected, Cougar returns the correct fault
 * "
 */
public class SOAPTestLargeGetERRORHANDLINGINVALIDTYPETest {
    ...
    public void doTest(boolean schemaValidationEnabled) throws Exception {
        ...
        try {
            ...
            // Create the SOAP request as an XML Document (with string value passed as int parameter)
            XMLHelpers xMLHelpers2 = new XMLHelpers();
            Document createAsDocument2 = xMLHelpers2.getXMLObjectFromString("<TestLargeGetRequest><size>x</size></TestLargeGetRequest>");
