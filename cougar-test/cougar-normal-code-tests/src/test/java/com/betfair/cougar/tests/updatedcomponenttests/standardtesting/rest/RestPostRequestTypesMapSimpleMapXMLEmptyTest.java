...
// Originally from UpdatedComponentTests/StandardTesting/REST/Rest_Post_RequestTypes_Map_SimpleMap_XML_Empty.xls;
...
/**
 * Ensure that when a Rest (XML) Post operation is performed, Cougar can correctly handle an empty simple Map in the post body
 */
public class RestPostRequestTypesMapSimpleMapXMLEmptyTest {
    @Test
    public void doTest() throws Exception {
        ...
        // Set the post body to contain an empty simple map 
        getNewHttpCallBean1.setRestPostQueryObjects(DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new ByteArrayInputStream("<message><simpleMap/></message>".getBytes())));
        ...
        Document createAsDocument10 = xMLHelpers3.getXMLObjectFromString("<SimpleMapOperationResponse><SimpleMapOperationResponseObject><responseMap/></SimpleMapOperationResponseObject></SimpleMapOperationResponse>");
        // Create the expected response as a JSON object
        JSONHelpers jSONHelpers4 = new JSONHelpers();
        JSONObject createAsJSONObject11 = jSONHelpers4.createAsJSONObject(new JSONObject("{\"responseMap\": {}}"));
