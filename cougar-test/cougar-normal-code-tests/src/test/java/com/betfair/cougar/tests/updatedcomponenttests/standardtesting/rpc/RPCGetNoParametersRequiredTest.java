...
// Originally from UpdatedComponentTests/StandardTesting/RPC/RPC_Get_NoParametersRequired.xls;
...
/**
 * Ensure that when a Batched JSON request is performed against a Cougar operation that requires no parameters, the correct response is received.
 */
public class RPCGetNoParametersRequiredTest {
    @Test
    public void doTest() throws Exception {
        ...
        mapArray2[0].put("method","testNoParams");
        mapArray2[0].put("params","[]");
        mapArray2[0].put("id","1");
        mapArray2[1] = new HashMap();
        mapArray2[1].put("method","testNoParams");
        ...
        AssertionUtils.multiAssertEquals("{\"id\":1,\"result\":{\"status\":\"hello\",\"version\":\"1.0.0\"},\"jsonrpc\":\"2.0\"}", map5.get("response1"));
        AssertionUtils.multiAssertEquals("{\"id\":2,\"result\":{\"status\":\"hello\",\"version\":\"1.0.0\"},\"jsonrpc\":\"2.0\"}", map5.get("response2"));
        ...
        cougarManager.verifyRequestLogEntriesAfterDate(timeStamp, new RequestLogRequirement("2.8", "testNoParams"),new RequestLogRequirement("2.8", "testNoParams") );
