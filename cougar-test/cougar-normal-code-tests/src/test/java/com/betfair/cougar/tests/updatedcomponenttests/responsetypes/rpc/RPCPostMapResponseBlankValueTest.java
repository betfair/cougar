...
// Originally from UpdatedComponentTests/ResponseTypes/RPC/RPC_Post_MapResponse_BlankValue.xls;
...
/**
 * Ensure that when a Batched JSON request is performed against Cougar, passing in a Map containing a blank value in the post body, it is correctly de-serialized, processed, returned the correct response.
 */
public class RPCPostMapResponseBlankValueTest {
    @Test
    public void doTest() throws Exception {
        ...
        mapArray2[0].put("params","[{\"Key a\":\"\",\"Key b\":\"value for b\"}]");
        mapArray2[0].put("id","1");
        mapArray2[1] = new HashMap();
        mapArray2[1].put("method","testSimpleMapGet");
        mapArray2[1].put("params","[{\"Key c\":\"\",\"Key d\":\"value for d\"}]");
        ...
        AssertionUtils.multiAssertEquals("{\"id\":1,\"result\":{\"Key b\":\"value for b\",\"Key a\":\"\"},\"jsonrpc\":\"2.0\"}", map5.get("response1"));
        AssertionUtils.multiAssertEquals("{\"id\":2,\"result\":{\"Key d\":\"value for d\",\"Key c\":\"\"},\"jsonrpc\":\"2.0\"}", map5.get("response2"));
