...
// Originally from UpdatedComponentTests/ResponseTypes/RPC/RPC_Post_MapResponse.xls;
...
/**
 * Ensure that when a Batched JSON request is performed against Cougar, passing in a Map in the post body, it is correctly de-serialized, processed, returned the correct Map response.
 */
public class RPCPostMapResponseTest {
    @Test
    public void doTest() throws Exception {
        ...
        mapArray2[0].put("params","[{\"Key a\":\"value for a\",\"Key b\":\"value for b\"}]");
        mapArray2[0].put("id","1");
        mapArray2[1] = new HashMap();
        mapArray2[1].put("method","testSimpleMapGet");
        mapArray2[1].put("params","[{\"Key c\":\"value for c\",\"Key d\":\"value for d\"}]");
        ...
        AssertionUtils.multiAssertEquals("{\"id\":1,\"result\":{\"Key b\":\"value for b\",\"Key a\":\"value for a\"},\"jsonrpc\":\"2.0\"}", map5.get("response1"));
        AssertionUtils.multiAssertEquals("{\"id\":2,\"result\":{\"Key d\":\"value for d\",\"Key c\":\"value for c\"},\"jsonrpc\":\"2.0\"}", map5.get("response2"));
