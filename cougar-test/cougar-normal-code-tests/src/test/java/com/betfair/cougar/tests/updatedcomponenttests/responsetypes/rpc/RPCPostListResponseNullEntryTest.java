...
// Originally from UpdatedComponentTests/ResponseTypes/RPC/RPC_Post_ListResponse_NullEntry.xls;
...
/**
 * Ensure that when a Batched JSON request is performed against Cougar, passing a List with a null entry in the post body, it is correctly de-serialized, processed, returned the correct response.
 */
public class RPCPostListResponseNullEntryTest {
    @Test
    public void doTest() throws Exception {
        ...
        AssertionUtils.multiAssertEquals("{\"id\":1,\"result\":[\"Value a\",null],\"jsonrpc\":\"2.0\"}", map5.get("response1"));
        AssertionUtils.multiAssertEquals("{\"id\":2,\"result\":[\"Value c\",null],\"jsonrpc\":\"2.0\"}", map5.get("response2"));
