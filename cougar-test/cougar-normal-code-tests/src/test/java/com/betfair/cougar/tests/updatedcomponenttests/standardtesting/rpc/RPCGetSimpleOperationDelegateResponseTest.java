...
// Originally from UpdatedComponentTests/StandardTesting/RPC/RPC_Get_SimpleOperation_DelegateResponse.xls;
...
/**
 * Ensure that when a Batched JSON simple Get is performed against Cougar, the repsonse can be populated using a delegate.
 */
public class RPCGetSimpleOperationDelegateResponseTest {
    @Test
    public void doTest() throws Exception {
        ...
        mapArray2[0].put("method","testSimpleGetQA");
        mapArray2[0].put("params","[\"DELEGATE\"]");
        mapArray2[0].put("id","1");
        mapArray2[1] = new HashMap();
        mapArray2[1].put("method","testSimpleGetQA");
        mapArray2[1].put("params","[\"DELEGATE\"]");
        ...
        AssertionUtils.multiAssertEquals("{\"id\":1,\"result\":{\"message\":\"Hardcoded delegate response\"},\"jsonrpc\":\"2.0\"}", map5.get("response1"));
        AssertionUtils.multiAssertEquals("{\"id\":2,\"result\":{\"message\":\"Hardcoded delegate response\"},\"jsonrpc\":\"2.0\"}", map5.get("response2"));
        ...
        cougarManager.verifyRequestLogEntriesAfterDate(timeStamp, new RequestLogRequirement("2.8", "testSimpleGetQA"),new RequestLogRequirement("2.8", "testSimpleGetQA") );
