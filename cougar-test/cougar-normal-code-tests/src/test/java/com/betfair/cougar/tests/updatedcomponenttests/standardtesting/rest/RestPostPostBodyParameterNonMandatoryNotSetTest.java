...
// Originally from UpdatedComponentTests/StandardTesting/REST/Rest_Post_PostBody_Parameter_NonMandatory_NotSet.xls;
...
/**
 * Ensure that when a Rest (XML/JSON) Post operation is performed against Cougar, non-mandatory Post Body Params do not have to be set in the request or the response.
 */
public class RestPostPostBodyParameterNonMandatoryNotSetTest {
    @Test
    public void doTest() throws Exception {
        ...
        // Set the post body for the request, not setting one of the nonmandatory post body parameters
        getNewHttpCallBean1.setRestPostQueryObjects(DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new ByteArrayInputStream("<message><bodyParameter2>postBodyParamString2</bodyParameter2></message>".getBytes())));
        // Get current time for getting log entries later

        Timestamp getTimeAsTimeStamp9 = new Timestamp(System.currentTimeMillis());
        // Make the 4 REST calls to the operation
        cougarManager1.makeRestCougarHTTPCalls(getNewHttpCallBean1);
        // Create the expected response as an XML document (not expecting the missing nonmandatory body parameter)
        XMLHelpers xMLHelpers5 = new XMLHelpers();
        Document createAsDocument11 = xMLHelpers5.getXMLObjectFromString("<NonMandatoryParamsOperationResponseObject><bodyParameter2>postBodyParamString2</bodyParameter2><headerParameter>headerParamString</headerParameter><queryParameter>queryParamString</queryParameter></NonMandatoryParamsOperationResponseObject>");
        // Convert the expected response to REST types for comparison with actual responses
        Map<CougarMessageProtocolRequestTypeEnum, Object> convertResponseToRestTypes12 = cougarManager1.convertResponseToRestTypes(createAsDocument11, getNewHttpCallBean1);
        ...
        cougarManager1.verifyRequestLogEntriesAfterDate(getTimeAsTimeStamp9, new RequestLogRequirement("2.8", "nonMandatoryParamsOperation"),new RequestLogRequirement("2.8", "nonMandatoryParamsOperation"),new RequestLogRequirement("2.8", "nonMandatoryParamsOperation"),new RequestLogRequirement("2.8", "nonMandatoryParamsOperation") );
    }
