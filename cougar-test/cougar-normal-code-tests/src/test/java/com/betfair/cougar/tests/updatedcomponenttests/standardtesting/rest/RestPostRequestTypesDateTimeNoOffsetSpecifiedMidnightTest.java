...
// Originally from UpdatedComponentTests/StandardTesting/REST/Rest_Post_RequestTypes_DateTime_NoOffsetSpecified_Midnight.xls;
...
/**
 * Ensure that Cougar can handle the dateTime data type in the post body with no offset specified (and the time set to midnight)
 */
public class RestPostRequestTypesDateTimeNoOffsetSpecifiedMidnightTest {
    @Test
    public void doTest() throws Exception {
        ...
        String date = TimingHelpers.convertUTCDateTimeToCougarFormat((int) 2009, (int) 2, (int) 1, (int) 0, (int) 0, (int) 0, (int) 0);
        // Set the post body to contain a date time object
        getNewHttpCallBean1.setRestPostQueryObjects(DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new ByteArrayInputStream("<bodyParamDateTimeObject><dateTimeParameter>2009-02-01T00:00:00.000</dateTimeParameter></bodyParamDateTimeObject>".getBytes())));
        ...
        Document createAsDocument11 = xMLHelpers4.createAsDocument(DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(new ByteArrayInputStream(("<DateTimeOperationResponse><DateTimeOperationResponseObject><localTime>"+date+"</localTime><localTime2>"+date+"</localTime2></DateTimeOperationResponseObject></DateTimeOperationResponse>").getBytes())));
        ...
        JSONObject createAsJSONObject12 = jSONHelpers5.createAsJSONObject(new JSONObject("{\"localTime\":\""+date+"\",\"localTime2\":\""+date+"\"}"));
        ...
        AssertionUtils.multiAssertEquals(createAsJSONObject12, response7.getResponseObject());
