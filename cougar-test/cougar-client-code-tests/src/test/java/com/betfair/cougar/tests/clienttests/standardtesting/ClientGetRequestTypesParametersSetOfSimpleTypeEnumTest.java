...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Get_RequestTypes_Parameters_SetOfSimpleTypeEnum.xls;
...
/**
 * Ensure that when a simpleEnumSet is passed to cougar via a cougar client the request is sent and the response is handled correctly
 */
public class ClientGetRequestTypesParametersSetOfSimpleTypeEnumTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        Set<SimpleEnum> querySet = cougarClientResponseTypeUtils2.buildEnumSet("FOOFOO,BARBAR,FOOFOO");
        // Build set to pass as parameter
        CougarClientResponseTypeUtils cougarClientResponseTypeUtils3 = new CougarClientResponseTypeUtils();
        Set<SimpleEnum> headerSet = cougarClientResponseTypeUtils3.buildEnumSet("FOO,FOOBAR,BARFOO");
        // Make call to the method via client and validate the same sets have been echoed in response
        NonMandatoryParamsOperationResponseObject response4 = client.simpleEnumSetOperation(context, headerSet, querySet);
        assertEquals("BARBAR,FOOFOO", response4.getQueryParameter());
        assertEquals("BARFOO,FOO,FOOBAR", response4.getHeaderParameter());
    }
