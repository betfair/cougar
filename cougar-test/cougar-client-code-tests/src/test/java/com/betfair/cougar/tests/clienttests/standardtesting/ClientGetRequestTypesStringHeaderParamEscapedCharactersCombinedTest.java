...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Get_RequestTypes_String_HeaderParam_EscapedCharacters_Combined.xls;
...
/**
 * Ensure that when multiple escaped characters are passed in a header parameter to cougar via a cougar client the request is sent and the response is handled correctly
 */
public class ClientGetRequestTypesStringHeaderParamEscapedCharactersCombinedTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        NonMandatoryParamsOperationResponseObject response3 = client.nonMandatoryParamsOperation(context, "this & that is 100%", "queryParamString", body);
        assertEquals("queryParamString", response3.getQueryParameter());
        assertEquals("this & that is 100%", response3.getHeaderParameter());
    }
