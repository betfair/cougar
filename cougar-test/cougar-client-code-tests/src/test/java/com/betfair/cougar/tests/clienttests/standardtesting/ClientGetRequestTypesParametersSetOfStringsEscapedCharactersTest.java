...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Get_RequestTypes_Parameters_SetOfStrings_EscapedCharacters.xls;
...
/**
 * Ensure that when a stringSet with escaped characters is passed to cougar via a cougar client the request is sent and the response is handled correctly
 */
public class ClientGetRequestTypesParametersSetOfStringsEscapedCharactersTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        Set<String> querySet = cougarClientResponseTypeUtils2.buildSet("a query1,:query2,this & that is 100%");
        // Build set to pass as parameter
        CougarClientResponseTypeUtils cougarClientResponseTypeUtils3 = new CougarClientResponseTypeUtils();
        Set<String> headerSet = cougarClientResponseTypeUtils3.buildSet("a header1,:header2,this & that is 100%");
        ...
        assertEquals(":query2,a query1,this & that is 100%", response4.getQueryParameter());
        assertEquals(":header2,a header1,this & that is 100%", response4.getHeaderParameter());
    }
