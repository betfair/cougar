...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Post_RequestTypes_Floats_Zeroes.xls;
...
/**
 * Ensure that when a (Zeroe) Float object is passed in parameters to cougar via a cougar client, the request is sent and the response is handled correctly
 */
public class ClientPostRequestTypesFloatsZeroesTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        bodyParamFloatObject2.setBodyParameter((float) 0);
        ...
        FloatOperationResponseObject response3 = client.floatOperation(context, (float) 0, (float) 0, bodyParam);
        assertEquals((float) 0, response3.getQueryParameter());
        assertEquals((float) 0, response3.getHeaderParameter());
        assertEquals((float) 0, response3.getBodyParameter());
    }
