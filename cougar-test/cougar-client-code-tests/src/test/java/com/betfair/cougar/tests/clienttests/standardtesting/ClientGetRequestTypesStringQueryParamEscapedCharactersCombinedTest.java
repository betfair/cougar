...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Get_RequestTypes_String_QueryParam_EscapedCharacters_Combined.xls;
...
/**
 * Ensure that when multiple escaped characters are passed in a query parameter to cougar via a cougar client the request is sent and the response is handled correctly
 */
public class ClientGetRequestTypesStringQueryParamEscapedCharactersCombinedTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        SimpleResponse response3 = client.testParameterStylesQA(context, com.betfair.baseline.v2.enumerations.TestParameterStylesQAHeaderParamEnum.Foo, "this & that is 100%", convertedDate1);
        assertEquals("headerParam=Foo,queryParam=this & that is 100%,dateQueryParam="+helper.dateInUTC(convertedDate1), response3.getMessage());
    }
