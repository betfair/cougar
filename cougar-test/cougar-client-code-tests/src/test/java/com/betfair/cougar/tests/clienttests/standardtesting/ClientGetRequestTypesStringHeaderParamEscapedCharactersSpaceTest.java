...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Get_RequestTypes_String_HeaderParam_EscapedCharacters_Space.xls;
...
/**
 * Ensure that when escaped characters (a space) are passed in a header parameter to cougar via a cougar client the request is sent and the response is handled correctly
 */
public class ClientGetRequestTypesStringHeaderParamEscapedCharactersSpaceTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        NonMandatoryParamsOperationResponseObject response3 = client.nonMandatoryParamsOperation(context, "a space", "queryParamString", body);
        assertEquals("queryParamString", response3.getQueryParameter());
        assertEquals("a space", response3.getHeaderParameter());
    }
