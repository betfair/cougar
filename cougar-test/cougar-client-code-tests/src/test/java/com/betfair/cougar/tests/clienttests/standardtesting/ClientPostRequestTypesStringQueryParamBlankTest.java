...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Post_RequestTypes_String_QueryParam_Blank.xls;
...
/**
 * Ensure that when an operation with a mandatory query param is performed against cougar via a cougar client with a blank query param, the request is sent and the response is handled correctly
 */
public class ClientPostRequestTypesStringQueryParamBlankTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        // Make call to the method via client (with blank query parameter) and validate the response is as expected
        MandatoryParamsOperationResponseObject response3 = client.mandatoryParamsOperation(context, "headerParamString", "", bodyParam);
        assertEquals("", response3.getQueryParameter());
