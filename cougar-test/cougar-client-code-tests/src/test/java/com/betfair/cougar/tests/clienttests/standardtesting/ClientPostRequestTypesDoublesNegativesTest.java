...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Post_RequestTypes_Doubles_Negatives.xls;
...
/**
 * Ensure that when a Negative Double object is passed in parameters to cougar via a cougar client, the request is sent and the response is handled correctly
 */
public class ClientPostRequestTypesDoublesNegativesTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        bodyParamDoubleObject2.setBodyParameter(-4.0);
        ...
        DoubleOperationResponseObject response3 = client.doubleOperation(context, -54.43223, -3.343432, bodyParam);
        assertEquals(-3.343432, response3.getQueryParameter());
        assertEquals(-54.43223, response3.getHeaderParameter());
        assertEquals(-4.0, response3.getBodyParameter());
    }
