...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Post_RequestTypes_Set_SimpleSet_Empty.xls;
...
/**
 * Ensure that when an empty SimpleSet object is passed in parameters to cougar via a cougar client, the request is sent and the response is handled correctly
 */
public class ClientPostRequestTypesSetSimpleSetEmptyTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        Set<String> inputSet = cougarClientResponseTypeUtils2.buildSet("");
        ...
        // Validate the received set is as expected (empty)
        assertEquals(new LinkedHashSet(Arrays.asList("")), result.getResponseSet());
    }
