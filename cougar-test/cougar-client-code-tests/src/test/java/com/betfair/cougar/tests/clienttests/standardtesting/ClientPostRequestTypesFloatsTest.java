...
// Originally from ClientTests/Transport/StandardTesting/Client_Rescript_Post_RequestTypes_Floats.xls;
...
/**
 * Ensure that when a Float object is passed in parameters to cougar via a cougar client, the request is sent and the response is handled correctly
 */
public class ClientPostRequestTypesFloatsTest {
    ...
    public void doTest(CougarClientWrapper.TransportType tt) throws Exception {
        ...
        bodyParamFloatObject2.setBodyParameter((float) 6.0E-4);
        ...
        FloatOperationResponseObject response3 = client.floatOperation(context, (float) 3.4028E+38, (float) 553.36, bodyParam);
        assertEquals((float) 553.36, response3.getQueryParameter());
        assertEquals((float) 3.4028E+38, response3.getHeaderParameter());
