...
<#assign retType=""><#t>
    ...
    <#elseif paramType == "dateTime" > <#t>
        <#assign retType = "" ><#t>
    <#else> <#t>
        <#assign retType="objectProjector("+paramType+"ClientCO.class)" ><#t>
    </#if><#t>
<#return retType><#t>
</#function><#t>
// Generated from clientConnectedDataType.ftl
package ${package}.${majorVersion}.co.client;
 ...
 */
public class ${service}ClientCO implements ${service}CO {
...
<#assign dataType=parseDataType(doc, interface)>
    ...
    public ${service}ClientCO(ObjectNode objectNode) {
        this.objectNode = objectNode;
    }

    public static ${service}ClientCO rootFrom(Heap heap) {
        return objectProjector(${service}ClientCO.class).project(heap.getRoot());
    }

    public static HListComplex<${service}ClientCO> rootFromAsList(Heap heap) {
        return (ComplexListProjection<${service}ClientCO>) listProjector(objectProjector(${service}ClientCO.class)).project(heap.ensureRoot(NodeType.LIST));
    }

    public static HMapComplex<${service}ClientCO> rootFromAsMap(Heap heap) {
        return (ComplexMapProjection<${service}ClientCO>) mapProjector(objectProjector(${service}ClientCO.class)).project(heap.ensureRoot(NodeType.MAP));
    }
...
<#macro parameter><#t>
    ...
    <#elseif (paramType?starts_with("list(") || paramType?starts_with("set(")) && paramType?ends_with(")")><#t>
        ...
        <#assign javaType=translateTypes2(subType, interface, true, "ClientCO", true, false) ><#t>
    ...
    <#elseif paramType?starts_with("map(") && paramType?ends_with(")")><#t>
        ...
        <#assign keyType = paramType?substring(4, comma)?trim><#t>
        <#assign valueType = paramType?substring(comma+1, paramType?length-1)?trim><#t>
        <#assign javaType=translateTypes2(valueType, interface, true, "ClientCO", true, false) ><#t>
        <#if javaType?starts_with("ScalarProjection<") >
            <#assign javaType=javaType?substring(17,javaType?length-1) ><#t>
    ...
    <#elseif paramType == "dateTime" > <#t>
        ...
        <#assign returnTypeIsObjectOrCollection = false ><#t>
    <#else> <#t>
        <#assign javaType=paramType+"ClientCO" ><#t>
        <#assign returnType = javaType ><#t>
        <#assign returnTypeIsObjectOrCollection = true ><#t>
    ...
    public @Override ${returnType} get${cappedParam}() {
        Node node = objectNode.getField("${param}");
        if (node == null) {
            return null;
        }
        return node.project(${factoryName});
    }
    <#else>
    ...
    public @Override ${returnType} get${cappedParam}() {
        ...
        return node.project(${factoryName}).get();
    }
    </#if>
