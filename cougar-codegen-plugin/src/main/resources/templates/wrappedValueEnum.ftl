...
// Generated from wrappedValueEnum.ftl
...
public enum ${name?cap_first}WrappedValueEnum implements Externalizable, TranscribableEnum {
	<#assign first=true>
	<@compress single_line=true><#recurse doc>,UNRECOGNIZED_VALUE(null);</@compress>

    private static Set<${name?cap_first}WrappedValueEnum> validValues = Collections.unmodifiableSet(EnumSet.complementOf(EnumSet.of(${name?cap_first}WrappedValueEnum.UNRECOGNIZED_VALUE)));
    public static Set<${name?cap_first}WrappedValueEnum> validValues() { return validValues; }
    
    private String value;

	private ${name?cap_first}WrappedValueEnum(String value) {
		this.value=value;
	}

	private ${name?cap_first}WrappedValueEnum(int id) {
