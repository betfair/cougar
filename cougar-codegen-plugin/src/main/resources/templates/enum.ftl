...
public enum ${name?cap_first}Enum implements Externalizable, TranscribableEnum {
	<#assign first=true>
	<@compress single_line=true><#recurse doc>,UNRECOGNIZED_VALUE(null);</@compress>

    private static Set<${name?cap_first}Enum> validValues = Collections.unmodifiableSet(EnumSet.complementOf(EnumSet.of(${name?cap_first}Enum.UNRECOGNIZED_VALUE)));
    public static Set<${name?cap_first}Enum> validValues() { return validValues; }
    
    private String value;

	private ${name?cap_first}Enum(String value) {
		this.value=value;
	}

	private ${name?cap_first}Enum(int id) {
